...
<body class="fix-header fix-sidebar">
																	...
																	while($rows=mysqli_fetch_array($query))
																		{
																				...
																				$mql="SELECT * FROM res_category where c_id='".$rows['c_id']."'";
																					...
																					echo ' <tr id="restaurant-row-'.$rows['rs_id'].'"><td>'.$row['c_name'].'</td>
																								<td>'.$rows['title'].'</td>
																								<td>'.$rows['email'].'</td>
																								<td>'.$rows['phone'].'</td>
																								<td>'.$rows['url'].'</td>
																								...
																								<td>'.$rows['date'].'</td>
																									 <td><a href="javascript:void(0);" onclick="deleteRestaurant('.$rows['rs_id'].')" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i class="fa fa-trash-o" style="font-size:16px"></i></a> 
    ...
    function deleteRestaurant(restaurantId) {
        if(confirm('Are you sure you want to delete this restaurant? This will also delete all associated menu items.')) {
            $.ajax({
                url: 'delete_restaurant.php?res_del=' + restaurantId,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if(response.success) {
                        $('#restaurant-row-' + restaurantId).fadeOut(300, function() {
                            $(this).remove();
                        });
                ...
                error: function(xhr, status, error) {
                    alert('Error deleting restaurant. Please try again. Status: ' + status);
                }
            });
        }
    }
